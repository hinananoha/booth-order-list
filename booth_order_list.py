...
parser.add_argument('-f', '--file', help='BOOTHから出力された宛名印刷用CSVファイルのファイル名（必須）', required=True)
parser.add_argument('-o', '--output', help='出力するCSVファイルのファイル名（必須）', required=True)
parser.add_argument('-u', '--unshipped', action='store_true', help='未発送の注文のリストを出力')
parser.add_argument('-c', '--current-month', action='store_true', help='今月の注文のリストを出力')
parser.add_argument('-r', '--range', nargs=2 ,help='指定した期間の注文リストを出力(YYYY-MM-DD YYYY-MM-DDで指定)')
